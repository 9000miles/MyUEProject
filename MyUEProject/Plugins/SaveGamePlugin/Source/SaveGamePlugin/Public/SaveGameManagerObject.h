...
#include "SaveGameManagerObject.generated.h"

/*
代码中添加、保存、修改，需要先Load一次
*/
UCLASS(Blueprintable, BlueprintType)
class SAVEGAMEPLUGIN_API USaveGameManagerObject : public UObject, public FTickableGameObject
{
	GENERATED_BODY()
...
public:
	UFUNCTION(BlueprintPure)
		static USaveGameManagerObject* GetSaveGameManagerEditor();

	//UFUNCTION(BlueprintPure, Category = "SaveGameManager", meta = (WorldContext = "ContextObject"))
	//	static USaveGameManagerObject* GetSaveGameManager(const UObject* ContextObject);
private:
	void UpdateGroupSaveDataMap();
	static USaveGameManagerObject* Singleton;
};